...
export const isPageStory = (storyId: string): boolean => PAGE_REGEX.test(storyId);
...
export function summarizeIndex(storyIndex: StoryIndex): { storyCount: number; componentCount: number; pageStoryCount: number; playStoryCount: number; testStoryCount: number; maxTestsPerStory: number; singleTestStoryCount: number; autodocsCount: number; mdxCount: number; exampleStoryCount: number; exampleDocsCount: number; onboardingStoryCount: number; onboardingDocsCount: number; svelteCsfV4Count: number; svelteCsfV5Count: number; version: number; } {
  let storyCount = 0;
