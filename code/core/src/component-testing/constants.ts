export const ADDON_ID = 'storybook/interactions';
export const PANEL_ID: "storybook/interactions/panel" = `${ADDON_ID}/panel`;
export const PARAM_KEY = 'interactions';

export const DOCUMENTATION_LINK = 'writing-tests/integrations/vitest-addon';
export const DOCUMENTATION_DISCREPANCY_LINK: "writing-tests/integrations/vitest-addon#what-happens-when-there-are-different-test-results-in-multiple-environments" = `${DOCUMENTATION_LINK}#what-happens-when-there-are-different-test-results-in-multiple-environments`;
export const DOCUMENTATION_PLAY_FUNCTION_LINK =
  'writing-stories/play-function#writing-stories-with-the-play-function';

export const INTERNAL_RENDER_CALL_ID = 'internal_render_call';
