...
export class PreviewWithSelection<TRenderer extends Renderer> extends Preview<TRenderer> {
  ...
  async onUpdateArgs({ storyId, updatedArgs }: { storyId: StoryId; updatedArgs: Args }): Promise<void> {
  ...
  mainStoryCallbacks(storyId: StoryId) {
    return {
      ...
      showError: (err: { title: string; description: string }): void => this.renderError(storyId, err),
  ...
  renderError(storyId: StoryId, { title, description }: { title: string; description: string }): void {
    const renderId = this.currentRender?.renderId;
